...
import com.allmuslim.request.PraytimeRequest;
...
import com.allmuslim.util.GetCalcValue;
import com.mysql.cj.log.LogFactory;


@RestController
@RequestMapping("/praytime")
public class PrayTimeController {
	
	@Autowired
	private PrayerTimeService prayerTimeService;
	
	private static final Logger log=LoggerFactory.getLogger(PrayTimeController.class);
	// ---------------------- Global Variables --------------------
	 ...
	 public PrayTimeController() {
		// TODO Auto-generated constructor stub
	      ...
	      //List<CalcValResponse> listCalcVal=GetCalcValue.calcValResponse();
	      List<CalcValResponse> listCalcVal=GetCalcValue.calcValResponse();
	     for(CalcValResponse itr:listCalcVal)
	     {
	    	 double[] Jvalues = {itr.getVal1(),itr.getVal2(),itr.getVal3(),itr.getVal4(),itr.getVal5()};
		      methodParams.put(Integer.valueOf(itr.getOrderid()), Jvalues);
	     }
	      // Jafari
	...
	@RequestMapping(value="getPrayTime",method=RequestMethod.POST)
	public List<String> getPrayTime(@RequestBody PraytimeRequest prayTimeRequest)
	{
		log.info("--------------test controller----------------"+prayTimeRequest.getLatitude());
		CalcMethodResponse calcMethodResp=prayerTimeService.getCalTypeByCountry(prayTimeRequest.getCountry());
	
	      PrayTimeController prayers = new PrayTimeController();

	      prayers.setTimeFormat(prayers.Time12);
	     // prayers.setCalcMethod(prayers.Jafari);
	      //System.out.println("+++++++++++"+prayers.Karachi);
	      prayers.setCalcMethod(calcMethodResp.getCalc_value());
	      ...
	      ArrayList<String> prayerTimes = prayers.getPrayerTimes(cal,
	    		  prayTimeRequest.getLatitude(), prayTimeRequest.getLongitude(), calcMethodResp.getTimezone());
	      ArrayList<String> prayerNames = prayers.getTimeNames();
	      ArrayList<String> list=new ArrayList<String>();
	      for (int i = 0; i < prayerTimes.size(); i++) {
	    	 // list=new ArrayList<String>();
	    	  list.add(prayerNames.get(i) + " - " + prayerTimes.get(i));
